...
/**
 * Fetch vendors from Firestore for a given organization.
 * Server-side only; returns an array of vendor documents.
 */
export async function getVendorsFromDb(orgId: string) {
  const { getFirebaseAdmin } = await import("./firebase-admin");
  const { db } = getFirebaseAdmin();
  const snap = await db.collection("vendors").where("orgId", "==", orgId).get();
  return snap.docs.map((d: any) => ({ id: d.id, ...d.data() }));
}

/**
 * Fetch a single vendor by id from Firestore (server-side only).
 */
...
/**
 * Create a vendor document in Firestore. Expects the caller to ensure
 * `vendor.orgId` matches the caller's org and that this runs on server.
 */
export async function createVendorInDb(
  ...
  const { getFirebaseAdmin } = await import("./firebase-admin");
  const { db } = getFirebaseAdmin();
  const ref = await db.collection("vendors").add(vendor as any);
  const doc = await ref.get();
...
export async function getRolesFromDb(orgId: string) {
  const { getFirebaseAdmin } = await import("./firebase-admin");
  const { db } = getFirebaseAdmin();
  const snap = await db.collection("roles").where("orgId", "==", orgId).get();
  return snap.docs.map((d: any) => ({ id: d.id, ...d.data() }));
}

/**
 * Create or update a role document. Caller must be server/admin.
 */
export async function upsertRoleInDb(
  ...
  if (role.id) {
    ...
    await ref.set(role as any, { merge: true });
    const doc = await ref.get();
    return { id: doc.id, ...doc.data() };
  }
  const ref = await db.collection("roles").add(role as any);
  const doc = await ref.get();
  return { id: ref.id, ...doc.data() };
}
...
/**
 * Server-side: create a purchase order in Firestore.
...
export async function createPurchaseOrderInDb(
  ...
  try {
    // Minimal server-side implementation that writes to Firestore when admin is available.
    const { getFirebaseAdmin } = await import("./firebase-admin");
    const { db } = getFirebaseAdmin();
    const data = {
      ...po,
    ...
    const ref = await db.collection("purchaseOrders").add(data);
    await createAuditLog({
      userId: userContext.user.id,
      userName: userContext.user.name,
      action: "create_order",
      ...
      entityId: ref.id,
      details: { vendorId: po.vendorId, totalAmount: po.totalAmount },
      ipAddress: "127.0.0.1",
    });
    return { success: true, id: ref.id };
  } catch (err: any) {
    console.error("createPurchaseOrderInDb failed", err);
    // Fallback to mock behavior to keep the UI functional in non-admin environments.
