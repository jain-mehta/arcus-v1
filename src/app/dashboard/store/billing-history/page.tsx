...
export default async function BillingHistoryPage() {
    ...
    const customersResult = await getSalesCustomers();
    const orders = (ordersResult.success ? ordersResult.data || [] : []) as any[];
    const customers = (customersResult.success ? customersResult.data || [] : []) as any[];
    const store = null; // Store data would come from a dedicated function if needed

    // Filter orders to only show those created from the current user's store
    const storeOrders = orders.filter(o => (o as any).store_id === user?.orgId || (o as any).storeId === user?.orgId);
